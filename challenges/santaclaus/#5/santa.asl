// Santa: the leader-agent at his workshop, assisting reindeers in toy delivery and elves in toy development

round(0).

+!group(Kind,G,O) : .desire(work(_,_,_))  // if santa is busy, goal !group will be resumed after !work
   <- .print("Waiting to proceed with ",G, " ...");
      +g(Kind,G,O);
      .suspend;                      
      !!group(Kind,G,O).               
@lg[atomic]  // santa cannot handle another event +!group (from resume) before executing !!work
+!group(Kind,G,O)
   <- +g(Kind,G,O);
      !!work(Kind,G,O).  // use !! so that work() isn’t performed as atomic

+!work(Kind,G,O) : round(N) & N == 10
   <- .print("Taking a break...");
      .wait(100);
      -+round(N+1);
      !!work(Kind,G,O).
+!work(Kind,G,O) : round(N) & N < 20  // just a stop condition
   <- .print("Working with ",Kind,"s: ",G," for ", O, "minutes (",N,")");
      -+round(N+1);
      .abolish(done);
      .send(G,achieve,proceed);
      .wait(.count(done[source(_)],.length(G)));  // suspends this intention until |G| dones are received
      .print("Group ",G," of ",Kind,"s has finished");
      -g(Kind,G,O);
      !!resume_all.  // .resume() needs to be in another intention, otherwise st claus will be working while checking new groups)
+!work(_,_,_) <- .print("It is better to stop for now...").

+!resume_all : .findall(O,g(Kind,G,O),LO) & LO \== []
   <- .print("Offers are ",LO);
      .min(LO,WO);
      .resume(group(WKind,WG,WO));  // min-value group have priority
      .resume(group(_,_,_)).
+!resume_all
   <- .resume(group(elf,_,_));
      .resume(group(_,_,_)).